...
6. **Fuzzy find ambiguity.** `--find "2"` may match "2", "Memory", "Result" etc. Add `--role Button` and check the `score` (1.0 = exact label match). If the top hit is not the control you want, refine the query or use `--list` to see all labels.
7. **UIA COM first run.** `comtypes` generates typelib bindings on first use — the first `uia_probe` call may take a few seconds. Subsequent calls are fast.
